...
import best.leafyalex.gainzclient.utils.Extensions.getVelocity
import best.leafyalex.gainzclient.utils.Extensions.scale
...
object WorldUtils {
    ...
    fun blockInFront(player: EntityPlayer, distance: Float, yMod: Float = 0f): Block { // yMod = 0 -> feet, 1 -> 1 above feet etc
        ...
        return GainzClient.mc.theWorld.getBlockState(player.position.add(vec.xCoord, -0.2 + yMod, vec.zCoord)).block
    }
    ...
    fun entityOffEdge(player: EntityPlayer): Boolean {
        if (!player.onGround) {
            ...
            for (position in positions) {
                if (GainzClient.mc.theWorld.getBlockState(BlockPos(position)).block != Blocks.air) {
                    return false
                }
            }
            return true
    ...
    private fun airCheck(player: EntityPlayer, pos: BlockPos, distance: Float, lookVec: Vec3): Boolean {
        for (i in 1..distance.toInt()) {
            if (GainzClient.mc.theWorld.getBlockState(BlockPos(pos.x + lookVec.xCoord * i, pos.y - (if (player.onGround) 0.2 else 1.4), pos.z + lookVec.zCoord * i)).block == Blocks.air) {
                return true
            }
        }
        return false
    ...
    private fun barrierCheck(player: EntityPlayer, pos: BlockPos, distance: Float, lookVec: Vec3): Boolean {
        for (i in 1..distance.toInt()) {
            if (GainzClient.mc.theWorld.getBlockState(BlockPos(pos.x + lookVec.xCoord * i, pos.y - (if (player.onGround) 0.2 else 1.4), pos.z + lookVec.zCoord * i)).block == Blocks.barrier) {
                return true
            }
        }
        return false
