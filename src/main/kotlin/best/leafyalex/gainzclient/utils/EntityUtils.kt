...
object EntityUtils {
    ...
    fun getOpponentEntity(): EntityPlayer? {
        if (GainzClient.mc.theWorld != null) {
            for (entity in GainzClient.mc.theWorld.playerEntities) {
                if (entity.displayName != GainzClient.mc.thePlayer.displayName && shouldTarget(entity)) {
                    return entity
                }
            }
        }
    ...
    private fun shouldTarget(entity: EntityPlayer?): Boolean {
        return if (entity == null) {
            false
        } else if (GainzClient.mc.thePlayer.isEntityAlive && entity.isEntityAlive) {
            if (!entity.isInvisible /*&& !entity.isInvisibleToPlayer(mc.thePlayer)*/) {
                GainzClient.mc.thePlayer.getDistanceToEntity(entity) <= 64.0f
            } else {
                false
            }
        } else {
    ...
    private fun getClosestCorner(corner1: Vec3, corner2: Vec3, corner3: Vec3, corner4: Vec3): ArrayList<Vec3> {
        val pos = Vec3(GainzClient.mc.thePlayer.posX, GainzClient.mc.thePlayer.posY + GainzClient.mc.thePlayer.eyeHeight, GainzClient.mc.thePlayer.posZ)
